...
namespace WIN32APIWapper
{
    ...
    public sealed class ShellItem : IEnumerable, IDisposable, IComparable
    {
        ...
        internal ShellItem(ShellBrowser browser, IntPtr pidl, IntPtr shellFolderPtr)
        {
            ...
            Shell32Wrapper.SHGetFileInfo(pidlRel.Ptr, 0, ref info, Shell32Wrapper.cbFileInfo,
        ...
        private static void SetText(ShellItem item)
        {
            ...
            if (item.ParentItem.ShellFolder.GetDisplayNameOf(
                            item.PIDLRel.Ptr,
                            SHGNO.INFOLDER,
                            strr) == Shell32Wrapper.S_OK)
            {
                ShlwapiWrapper.StrRetToBuf(strr, item.PIDLRel.Ptr, buf, Shell32Wrapper.MAX_PATH);
        ...
        private static void SetPath(ShellItem item)
        {
            ...
            if (item.ParentItem.ShellFolder.GetDisplayNameOf(
                            item.PIDLRel.Ptr,
                            SHGNO.FORADDRESSBAR | SHGNO.FORPARSING,
                            strr) == Shell32Wrapper.S_OK)
            {
                ShlwapiWrapper.StrRetToBuf(strr, item.PIDLRel.Ptr, buf, Shell32Wrapper.MAX_PATH);
        ...
        private static void SetInfo(ShellItem item)
        {
            ...
            SHFILEINFO info = new SHFILEINFO();
            Shell32Wrapper.SHGetFileInfo(pidlFull.Ptr, 0, ref info, Shell32Wrapper.cbFileInfo,
                SHGFI.PIDL | SHGFI.TYPENAME | SHGFI.SYSICONINDEX);
                ...
                try
                {
                    ...
                    if (expandFiles)
                    {
                        if (this.Equals(browser.DesktopItem) || parentItem.Equals(browser.DesktopItem))
                        {
                            if (ShellFolder.EnumObjects(
                                    winHandle,
                                    fileFlag,
                                    out fileEnumPtr) == Shell32Wrapper.S_OK)
                            {
                                fileEnum = (IEnumIDList)Marshal.GetTypedObjectForIUnknown(fileEnumPtr, typeof(IEnumIDList));
                                SFGAO attribs = SFGAO.FOLDER;
                                while (fileEnum.Next(1, out pidlSubItem, out celtFetched) == Shell32Wrapper.S_OK && celtFetched == 1)
                                {
                    ...
                    if (expandFolders)
                    {
                        if (ShellFolder.EnumObjects(
                                    winHandle,
                                    folderFlag,
                                    out folderEnumPtr) == Shell32Wrapper.S_OK)
                        {
                            folderEnum = (IEnumIDList)Marshal.GetTypedObjectForIUnknown(folderEnumPtr, typeof(IEnumIDList));
                            while (folderEnum.Next(1, out pidlSubItem, out celtFetched) == Shell32Wrapper.S_OK && celtFetched == 1)
                            {
                                IntPtr shellFolderPtr;
                                if (ShellFolder.BindToObject(
                                            pidlSubItem,
                                            IntPtr.Zero,
                                            ref Shell32Wrapper.IID_IShellFolder,
                                            out shellFolderPtr) == Shell32Wrapper.S_OK)
                                {
                    ...
                    try
                    {
                            ...
                            if (this.Equals(browser.DesktopItem) || parentItem.Equals(browser.DesktopItem))
                            {
                                        ...
                                        out fileEnumPtr) == Shell32Wrapper.S_OK)
                                {
                                    ...
                                    while (browser.UpdateCondition.ContinueUpdate &&
                                           fileEnum.Next(1, out pidlSubItem, out celtFetched) == Shell32Wrapper.S_OK && celtFetched == 1)
                                    {
                                        ShellFolder.GetAttributesOf(1, new IntPtr[] { pidlSubItem }, ref attribs);
                        ...
                        if (browser.UpdateCondition.ContinueUpdate && updateFolders)
                        {
                                        ...
                                        out folderEnumPtr) == Shell32Wrapper.S_OK)
                            {
                                folderEnum = (IEnumIDList)Marshal.GetTypedObjectForIUnknown(folderEnumPtr, typeof(IEnumIDList));
                                while (browser.UpdateCondition.ContinueUpdate &&
                                       folderEnum.Next(1, out pidlSubItem, out celtFetched) == Shell32Wrapper.S_OK && celtFetched == 1)
                                {
